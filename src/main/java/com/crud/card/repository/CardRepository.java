...
public class CardRepository implements ICardRepository{
    ...
    public int save(Card card) {
        ...
        return jdbcTemplate.update(SQL, new Object[]{card.getFullName(), card.getDocumentNumber(), card.getPhone(),
                card.getDate(),card.getStatus()});
    }

    @Override
    public int update(Card card) {
        String SQL = "UPDATE cards SET fullName =?, number = ?, phone=?, date=? WHERE id_card = ?";
        return jdbcTemplate.update(SQL, new Object[]{card.getFullName(), card.getDocumentNumber(), card.getPhone(),
                card.getDate(),card.getId_card()});
    }
